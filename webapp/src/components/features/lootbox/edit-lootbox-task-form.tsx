// components/features/lootbox/edit-lootbox-task-form.tsx
"use client";
...
// Схема валидации для редактирования LootBox задач
const editLootBoxTaskSchema = z.object({
...
export type EditLootBoxTaskFormData = z.infer<typeof editLootBoxTaskSchema>;

export interface EditLootBoxTaskFormRef {
  submitForm: () => void;
  resetForm: () => void;
}

interface EditLootBoxTaskFormProps {
  task: LootBoxTask;
  onSubmit: (data: EditLootBoxTaskFormData) => void;
  onValidationChange?: (isValid: boolean) => void;
}

const EditLootBoxTaskForm = forwardRef<
  EditLootBoxTaskFormRef,
  EditLootBoxTaskFormProps
>(({ task, onSubmit, onValidationChange }, ref) => {
  ...
  const {
    ...
    reset,
  } = useForm<EditLootBoxTaskFormData>({
    resolver: zodResolver(editLootBoxTaskSchema),
    mode: "onChange",
    defaultValues: {
      title: task.title || "",
  ...
  return (
    <div>
      <form id="edit-lootbox-task-form" onSubmit={handleSubmit(onSubmit)}>
...
EditLootBoxTaskForm.displayName = "EditLootBoxTaskForm";

export default EditLootBoxTaskForm;
